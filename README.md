...
**Binary Loss**
입력받은 image data가 fake인지 real인지 구분하는 loss이며 예측 정도를 T/F 예측을 0~1 사이의 값으로 출력한다. 
true or false 두가지 category를 비교하게 되므로 이진분류에 쓰이는 Binary Cross Entropy(BCE)를 사용한다.
